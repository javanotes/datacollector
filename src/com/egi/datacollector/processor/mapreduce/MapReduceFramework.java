...
import com.egi.datacollector.processor.mapreduce.pojo.EndProduceMsg;
import com.egi.datacollector.processor.mapreduce.pojo.InputCollectionMsg;
import com.egi.datacollector.processor.mapreduce.pojo.InputDataMsg;
import com.egi.datacollector.util.concurrent.mapreduce.MapReduceMasterActor;
		...
		if(!isAkkaInit){
			...
			masterActor = akka.actorOf(new Props(new UntypedActorFactory() {
				...
				public Actor create() {
					...
					return new MapReduceMasterActor<X, K, V>(map, reduce, results, _akLatch, partSize, mappers, reducers);
				}
			}));
			isAkkaInit = true;
